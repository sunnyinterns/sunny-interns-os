...
export async function GET(
  ...
  const shaped = (submissions ?? []).map(s => {
    const job = s.jobs as unknown as unknown as Record<string, unknown> | null
    const caseData = s.cases as unknown as unknown as Record<string, unknown> | null
    const internsRaw2 = caseData?.interns
    const intern = ((Array.isArray(internsRaw2) ? internsRaw2[0] : internsRaw2) as unknown as unknown as Record<string, unknown>) ?? {}
