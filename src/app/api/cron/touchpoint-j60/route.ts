...
export async function GET(req: Request) {
  ...
  // Find active cases where actual_start_date was 60 days ago
  ...
  const { data: cases } = await admin
    .from('cases')
    .select('id, portal_token, actual_start_date, actual_end_date, actual_start_date, touchpoint_j60_sent_at, interns(first_name, email)')
    .eq('status', 'active')
    .eq(actual_start_date, targetStr)
    .is(touchpoint_j60_sent_at, null)
    .limit(50)
  ...
  for (const cas of cases ?? []) {
    ...
    try {
      ...
      await admin.from('cases').update({
        touchpoint_j60_sent_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      }).eq('id', cas.id)
      sent.push(intern.email)
    } catch (e) {
      console.error('[touchpoint-j60]', intern.email, e)
    }
  }
