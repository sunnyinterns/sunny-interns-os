...
// nodejs runtime — edge broken with Next.js 16 + Turbopack
...
export async function GET(req: NextRequest) {
  ...
  return new ImageResponse(
    (
      // Satori: no z-index, no inset, no percentage widths inside absolute children
      <div style={{ width: W, height: H, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '50px 62px', fontFamily: 'system-ui, sans-serif', position: 'relative', backgroundImage: bgDataUri ? `url(${bgDataUri})` : undefined, backgroundSize: 'cover', backgroundPosition: 'center', background: bgDataUri ? undefined : 'linear-gradient(135deg,#1a1918 0%,#2d2518 50%,#c8a96e 100%)' }}>

          {/* Dark scrim — absolute but NO z-index, position after bg */}
          <div style={{ position: 'absolute', top: 0, left: 0, width: W, height: H, background: 'linear-gradient(to top, rgba(26,25,24,0.95) 38%, rgba(26,25,24,0.4) 68%, rgba(0,0,0,0.1) 100%)' }} />

          {/* Top row — needs to be after the absolute div to render on top */}
