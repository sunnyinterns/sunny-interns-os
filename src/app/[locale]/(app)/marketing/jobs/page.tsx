          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {tab === 'posts' && (
              <div className="space-y-3">
                {posts.length === 0 ? (
                  <div className="text-center py-12 text-zinc-400"><p className="text-3xl mb-2">✍️</p><p className="text-sm">Génère des posts depuis le Content Machine</p></div>
                ) : posts.map(post => {
                  const pd = PD[post.platform]
                  return (
                    <div key={post.id} className="bg-white border border-zinc-100 rounded-2xl overflow-hidden">
                      <div className="flex items-center gap-3 px-4 py-2 border-b border-zinc-50">
                        <span>{pd.icon}</span>
