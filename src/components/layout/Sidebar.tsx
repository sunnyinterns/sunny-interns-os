...
export function Sidebar() {
  ...
  useEffect(() => {
    fetch('/api/admin-notifications/count')
      ...
      .then((data) => { if (data) setUnreadNotifs(data.unread) })
      .catch(() => {})
    ...
    fetch('/api/cases?status=lead&count=true')
      .then((res) => res.ok ? res.json() as Promise<{ count: number } | unknown[]> : null)
      .then((data) => {
        if (data) {
          if (Array.isArray(data)) setNewLeadsCount(data.length)
          else if (typeof data === 'object' && 'count' in data) setNewLeadsCount((data as { count: number }).count)
        }
      })
    ...
    fetch('/api/cases?view=all')
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          const clientStatuses = ['visa_docs_sent', 'visa_submitted', 'visa_in_progress', 'visa_received', 'arrival_prep', 'active']
          const count = data.filter((c: any) => clientStatuses.includes(c.status)).length
          setActiveClientsCount(count)
        }
      })
      .catch(() => setActiveClientsCount(null))
  }, [])
  ...
  const navGroups: NavGroup[] = [
    {
      ...
      items: [
        ...
        },
        {
          href: '/fr/calendar',
          label: 'RDV Qualif',
          icon: <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /><circle cx="12" cy="16" r="1" fill="currentColor" /></svg>,
        },
      ],
    },
    {
      ...
      items: [
        {
          href: '/fr/pipeline',
          label: 'Pipeline',
          badgeCount: pipelineCount,
          badgeUrgent: hasTodos,
  ...
  const settingsSubNav: { href: string; label: string; icon: React.ReactNode }[] = [
    { href: '/fr/settings/notifications', label: 'Notifications', icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg> },
    { href: '/fr/settings/packages', label: 'Packages', icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10" /></svg> },
    { href: '/fr/settings/visa-types', label: 'Visas', icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg> },
    { href: '/fr/settings/visa-agents', label: 'Agents visa', icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg> },
    { href: '/fr/settings/partners', label: 'Partenaires', icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg> },
    { href: '/fr/settings/email-templates', label: 'Templates', icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg> },
    { href: '/fr/settings/housing', label: 'Logement', icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg> },
    { href: '/fr/settings/finances', label: 'Finances', icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
    { href: '/fr/settings/ugc', label: 'UGC', icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M15 10l4.553-2.069A1 1 0 0121 8.869v6.262a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg> },
    { href: '/fr/settings/users', label: 'Utilisateurs', icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg> },
    { href: '/fr/settings/templates', label: 'Contrats', icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg> },
    { href: '/fr/settings/schools', label: 'Écoles', icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M12 14l9-5-9-5-9 5 9 5z" /><path strokeLinecap="round" strokeLinejoin="round" d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" /></svg> },
  ]
      ...
      <nav className="flex-1 px-3 py-2 space-y-0.5 overflow-y-auto">
        {navGroups.map((group) => (
          <div key={group.title}>
            ...
            {group.items.map((item) => {
              ...
              const showBadge = item.badgeCount !== undefined && item.badgeCount !== null && item.badgeCount > 0
              return (
                <Link
                  key={item.href}
                  href={item.href}
                ...
                >
                  ...
                  {item.label}
                  {showBadge && (
                    <span className={`ml-auto flex-shrink-0 min-w-[18px] h-[18px] px-1 flex items-center justify-center rounded-full text-white text-[10px] font-semibold ${item.badgeUrgent ? 'bg-[#dc2626]' : 'bg-[#c8a96e]'}`}>
                      {item.badgeCount}
                    </span>
                  )}
                </Link>
        ...
        <div>
          ...
          >
            ...
            Paramètres
            {unreadNotifs > 0 && (
              <span className="ml-1 min-w-[18px] h-[18px] px-1 flex items-center justify-center rounded-full text-white text-[10px] font-semibold bg-[#dc2626]">
                {unreadNotifs}
              </span>
            )}
            <svg
