...
<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#fffbf0">
  ...
  <a href="https://bali-interns.com/apply" style="display:inline-block;background:#FFCC00;color:#1a1918;font-weight:700;padding:14px 28px;border-radius:100px;text-decoration:none;margin-top:16px">
...
export function newsletterMonthlyHtml(
  ...
  const jobsHtml = jobs.slice(0, 5).map(j => `
    <div style="border:1px solid #ede5d0;border-radius:12px;padding:16px 20px;margin-bottom:12px;background:#fff">
      ...
      ${j.slug ? `<a href="https://bali-interns.com/apply" style="display:inline-block;margin-top:10px;color:#b45309;font-size:13px;font-weight:600;text-decoration:none">Postuler →</a>` : ''}
    </div>`).join('')
